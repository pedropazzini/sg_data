...
class ts_cluster(object):
	...
	def k_means_clust(self,data,num_iter,w,progress=False):
		...
		for key in self.assignments:
	            ...
	            print(len(self.assignments[key]))
	...
	def plot_centroids_and_assignments(self, data,ini=None, end=None):
            cents = self.centroids
            total = len(cents)
            total_meters = len(data)
            i = 0
            for c in cents:
                assigns = self.assignments[i]
                name = "Centroid " + str(i+1) + "/" + str(total) + " ~ " + str(len(assigns)) + "/" + str(total_meters) +  " meters"
                plt.plot(c)
